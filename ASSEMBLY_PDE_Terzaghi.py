...
def create_mesh(mesh, cell_type, prune_z=False):
        ...
        return out_mesh
print('creando malla..')
geo = mshr.Rectangle(Point(-0.05, 0.0), Point(0.05,-0.1))
mesh = mshr.generate_mesh(geo, 100)
...
class bound(SubDomain):
        def inside(self, x, on_boundary):
            return on_boundary and abs(x[1]+0.1)< tol 
...
F2 = dt*inner(nabla_grad(q), K*nabla_grad(p))*dx \
     + alpha*(nabla_div(du_t))*q*dx + s_coef*(dp_t)*q*dx\
